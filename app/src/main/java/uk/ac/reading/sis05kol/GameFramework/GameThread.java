...
public abstract class GameThread extends Thread {
	...
	//Pre-begin a game
	abstract public void setupBeginning(int level_no);
	
	//Starting up the game
	public void doStart(int level_no) {
		this.level_no = level_no;
		synchronized(monitor) {
			
			setupBeginning(level_no);
	...
	/**
	 * Opens new levels or replays the level depending on the state of the game
	 *
	 * @param e
	 * @return
	 */
	public boolean onTouch(MotionEvent e) {
		...
		if (mMode == STATE_READY) {
			doStart(level_no); //start the level
			return true;
		}
		else if (mMode == STATE_LOSE) {
			doStart(level_no); //start the level again
			return true;
		}

		else if (mMode == STATE_WIN) {
			level_no += 1; //increment the level_no attribute
			doStart(level_no); //move onto the next level
			return true;
		}
